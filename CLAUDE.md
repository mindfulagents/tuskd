...
- **Cut a release (D19):** bump `[workspace.package] version` in `Cargo.toml` → merge to main (CI gate must be green) → `git tag v<V> && git push origin v<V>` → the cargo-dist workflow builds 4 targets and publishes the GitHub Release. Edit release config in `dist-workspace.toml` and regenerate `release.yml` with `dist generate` — never hand-edit the workflow.
